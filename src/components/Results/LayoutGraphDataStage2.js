 ...
 * All rights reserved.  
 *  
 * File Name: LayoutGraphDataStage2.js  
 * Description: This file is responsible to show stage 2 Layout Graph Data 
 *  
...
const LayoutGraphDataStage2 = ({ totalImages, projectName, columns, stage, currentProjectId, toggleSidebar, isSidebarCollapsed, setViewCartOption, setSelectedCellFromList, terminalHeight, gdsLayoutToggle, setGdsLayoutToggle, showGDSLayoutImageData, setShowGDSLayoutImageData, loadingRef }) => {



  const { stage2Result, stage2GdsLayoutImageData, setStage2GdsLayoutImageData } = useStage2ResultContext();
  const { stage2GraphData, setStage2GraphData } = useLayoutGraphContext();
  const { stage1FilterSelectedCells, setFilterRequestParameter, isFilterApplied, techVariationFilterSelectedParameter, stage2FilteredLevel2Cells, stage1IsAllCellSelectedForFilter, stage2TotalPexDataFilters, stage2Level2Filter, setStage2Level2Filter, setIsGraphFilterCleared, stage2GdsCartData, stage2GraphSelectedGdsCells, setStage2GraphSelectedGdsCells } = useFilterResultsContext();
  ...
  const [contextMenuOnStage2, setContextMenuOnStage2] = useState(false);
  const [selectedCellNameOnGraph, setSelectedCellNameOnGraph] = useState();
  ...
  //custom hooks
  const { handleCart, isGDSAddedToCart, handleClearCart } = useGdsLayoutCart(selectedCellNameOnGraph, 2);// sending 2 since user is setting cart for stage 2 hyperexpressivity
  ...
  const stage2GraphSelectedCellsRef = useRef(null);   // A ref to store cell selected in graph
  ...
  useEffect(()=>{
    if(stage2GdsLayoutImageData?.length === 0){
  ...
  if ((!hasFetchedImages || !gdsLayoutToggle) && stage2GraphData?.imagesData) {
  ...
  // Updating stage2graph data it the data is available and toggle is true.
  useEffect(() => {
    if (showGDSLayoutImageData && gdsLayoutToggle) {
      setStage2GraphData(prevState => ({
        ...prevState,
        imagesData: showGDSLayoutImageData || [],
        PEX_Consolidated: stage2Result?.data?.PEX_Consolidated || []
      }));
      setIsGDSImagesNeeded(true)
    } else{
      setIsGDSImagesNeeded(false);
    }
  }, [gdsLayoutToggle, setStage2GraphData, showGDSLayoutImageData, stage2Result])
  ...
  const downloadGDSFile = async () => {
    if (selectedCellNameOnGraph && currentProjectId) {
      ...
      const stage2GDSImageDownloadReq = {
        stage: 2,
      ...
      try {
        const stage2GDSImageDownloadRes = await downloadSingleGDSImage(stage2GDSImageDownloadReq);
        if (stage2GDSImageDownloadRes?.status === 200) {
          const blob = new Blob([stage2GDSImageDownloadRes?.data], { type: 'application/zip' });
          const url = window.URL.createObjectURL(blob);
          const link = document.createElement('a');
          link.href = url;

          // Extract filename from headers or set a default
          const contentDisposition = stage2GDSImageDownloadRes?.headers["x-filename"];
  ...
  const fetchData = useCallback(async () => {
    ...
    if (selectedOption1 && selectedOption2 && (!stage2GraphData || stage2GraphData?.PEX_Consolidated?.length === 0)) {
      setLoading(true);
      const mergedData = ({
        PEX_Consolidated: stage2Result?.data?.PEX_Consolidated || [],
        ...(showGDSLayoutImageData && { imagesData: showGDSLayoutImageData || [] })
      });
      setStage2GraphData(mergedData);
      setLoading(false);
      setConfirmLargeDataFetchModal(false);
    }
  ...
  }, [stage2Result, selectedOption1, selectedOption2, stage2GraphData, setStage2GraphData, showGDSLayoutImageData, gdsLayoutToggle]);

  const getLayoutDataList = useCallback(async () => {
    return stage2GraphData?.PEX_Consolidated?.flatMap(pexItem =>
      pexItem?.data?.map(item => item?.LayoutData)
    ) || [];
  }, [stage2GraphData]);

  const fetchDataTooltipImage = useCallback(async () => {
    if (!isGDSImagesNeeded && !stage2GraphData?.imagesData?.length) {

      // Directly use existing context data if available
      if (stage2GdsLayoutImageData && Array.isArray(stage2GdsLayoutImageData) && stage2GdsLayoutImageData.length > 0) {
        try {
          setStage2GraphData(prevState => ({
            ...prevState,
            imagesData: stage2GdsLayoutImageData || [],
          }));
        ...
        try {
          ...
          if (imagesData?.status && imagesData?.status_code === 200) {
            ...
            setStage2GdsLayoutImageData(imagesData?.images || []);
            setHasFetchedImages(true);
  ...
  }, [isGDSImagesNeeded, stage2GraphData, getLayoutDataList, setStage2GraphData, stage2GdsLayoutImageData, setStage2GdsLayoutImageData, gdsLayoutToggle, showGDSLayoutImageData, setShowGDSLayoutImageData, setGdsLayoutToggle]);

  const handleStage2GraphApplyFilter = useCallback(() => {
    setStage2GraphSelectedGdsCells(stage2GraphSelectedCellsRef.current);
  }, [setStage2GraphSelectedGdsCells]);

  // useEffect to trigger the function when context updates
  useEffect(() => {
    if (stage2GraphSelectedGdsCells !== null) {
     // setIsFilterApplied(true);
      handleApplyFilterStage2();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stage2GraphSelectedGdsCells]);
  
  // useEffect(() => {
  //   if (stage2GraphSelectedGdsCells) {
  //     handleApplyFilterStage2();
  //   }
  // }, [stage2GraphSelectedGdsCells]);
  ...
  const updateChart = useCallback((dataToRender = stage2GraphData?.PEX_Consolidated) => {
    ...
    if (!selectedOption1 || !selectedOption2 || !dataToRender) return;
    ...
    if (isGDSImagesNeeded) {
      stage2GraphData?.imagesData?.forEach(image => {
        imageMap[image.file] = image.encodedImage;  // Map image data
      });
    }
    ...
    dataToRender?.forEach(cell => {
      if (cell.data && Array.isArray(cell.data)) {
        cell.data.forEach(entry => {
          stage2Level2Filter?.forEach(({ parameterName1 }) => {
    ...
    dataToRender?.forEach(cell => {
      if (cell.data && Array.isArray(cell.data)) {
        cell.data.forEach(entry => {
          ...
          const passesFilters = stage2Level2Filter.every(({ parameterName1, value1, value2 }) => {
            const value = Number(entry[parameterName1]);
    ...
    const option = {
      tooltip: {
        ...
        formatter: function (params) {
          ...
          const isInCart = stage2GdsCartData?.includes(params?.name);
      ...
      toolbox: {
        feature: {
          ...
          myApplyGraphFilterButton: {
            ...
            onclick: handleStage2GraphApplyFilter
          },
          top: -10,
        }
      },
      ...
      series: dataToRender?.map(cell => ({
        ...
        data: cell?.data
          ?.filter(entry => {
            return stage2Level2Filter.every(({ parameterName1, value1, value2 }) => {
              const value = Number(entry[parameterName1]);
          ...
          .map(entry => {
            ...
            const isInCart = stage2GdsCartData?.includes(entry['File']);
            return {
              name: entry['File'],
    ...
    myChart?.current?.on('brushselected', function (params) {
      ...
      stage2GraphSelectedCellsRef.current = GDSCellNames;
    });    
    ...
    myChart?.current?.on('click', async function (params) {
      ...
      // Check if a point (scatter) was clicked and then proceed
      setSelectedCellNameOnGraph(params?.data?.name);
      setSelectedLayoutData([params?.data?.layoutData]);
      setMenuPosition({ x: xCoord, y: yCoord });
      setContextMenuOnStage2(true);
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedOption1, selectedOption2, stage2GraphData, generateRandomColor, projectName, isGDSImagesNeeded, setContextMenuOnStage2, setSelectedCellNameOnGraph, stage2Level2Filter, stage2GdsCartData]);

  const LargeDataFetchModalContent = {
    title: "Large Data Fetch",
    body: (
      <>
        <p>
  ...
  useEffect(() => {
    if (selectedOption1 && selectedOption2 && !stage2GraphData && totalImages > 500) {
      setConfirmLargeDataFetchModal(true);
    } else if (totalImages <= 500) {
      fetchData();
    }
  }, [selectedOption1, selectedOption2, stage2GraphData, totalImages, fetchData]);
  ...
  //     updateChart();
  //   }
  // }, [stage1GraphData, selectedOption1, selectedOption2, updateChart]);

  useEffect(() => {
    if (stage2Level2Filter) {
      updateChart();
    }
  }, [stage2Level2Filter, stage2TotalPexDataFilters, updateChart]);
  ...
  useEffect(() => {
    setNumericFields(findNumericFields(stage2GraphData?.PEX_Consolidated));
  }, [stage2GraphData]);
  ...
  //below useeffect will update filterrequest param as soon as filter is applied
  useEffect(() => {
    ...
    const filterRequest = {
      ...
      cells: isFilterApplied ? stage1FilterSelectedCells : stage2FilteredLevel2Cells?.length > 0 ? stage2FilteredLevel2Cells : [],

      filterParametersName: {
        filterLevel1: (isFilterApplied && filterParameters?.length > 0) ? filterParameters : null,
        filterLevel2: stage2Level2Filter ? stage2Level2Filter : null, // here compulsory level2 filter will set
      }
  ...
  const isLoading = loadingRef?.current || viewLayoutLoading;
  return (
    <div className="position-relative bg-white h-100">
      ...
      <div className="container-fluid h-100">
        <div className="row h-100">
          ...
          <div className={`${isGraphFilterSidebarOpen ? 'col-9' : 'col-12'} d-flex p-2 flex-column`}>
            {/* Controls Section */}
            {stage2GraphData?.PEX_Consolidated?.length > 0 && (
              <div className="row g-3 align-items-center justify-content-center" id="leftgraphview">
                  ...
                  <div className="bg-white d-flex align-items-center px-3 py-2 border rounded" style={{ height: '38px' }}>
                    <div className="form-check form-switch d-flex align-items-center m-0">
                      <input
                        ...
                        disabled={!(selectedOption1 && selectedOption2) && !stage2GraphData}
                        onChange={handleGDSTooltipImageToggle}
                        id="stage-2-tooltip-images-toggle"
                        style={{ cursor: 'pointer' }}
                      />
                      <label className="form-check-label" htmlFor="stage-2-tooltip-images-toggle">
                        Show GDS Layouts
                      </label>
                ...
                <div className={`${isGraphFilterSidebarOpen && !isSidebarCollapsed ? 'col-md-8' : 'col-md-6'} col-12 d-flex gap-3`}>
                  ...
                  {numericFields && (
                    <button
                      id="stage-2-graph-filter-button"
                      ...
                      data-tooltip-id="stage-2-graph-filter-btn-tooltip"
                      data-tooltip-content="Filters"
                      data-tooltip-place="top"
                    >
                      <FontAwesomeIcon icon={faFilter} className="text-white" />
                      <ReactTooltip id="stage-2-graph-filter-btn-tooltip" style={{ zIndex: 9999 }} />
                    </button>
                  )}
                </div>
              </div>
            ...
            {/* Chart Area */}
            <div ref={chartDom} className={`${isGraphFilterSidebarOpen ? 'col-9' : 'col-12'} row overflow-hidden`} id="stage-2-graph" style={{ position: 'absolute', height: '83%', bottom: '0' }} >
              {
                contextMenuOnStage2 && (
                  <ContextMenu
                    show={contextMenuOnStage2}
                    menuOptions={ContextMenuData}
                    onHide={() => setContextMenuOnStage2(false)}
          ...
          <div className={`bg-light p-2 h-100 overflow-auto ${isGraphFilterSidebarOpen ? 'col-3 d-block' : 'd-none'}`}>
            <div className="d-flex justify-content-between align-items-center mb-4">
              <h5 className="m-0 text-dark font-weight-bold">Filters</h5>
              <div>
                {/* below buttons are visible only if filter is applied */}
                {stage2Level2Filter?.length > 0 && (
                  <>
                ...
                >
                  ...
                  <ReactTooltip id="stage-1-graph-filter2-sync-across-tab-btn-tooltip" style={{ zIndex: 9999 }} />
                    ...
                    <button
                      className="btn btn-link button-primary-color p-0 me-3"
                      id="stage-2-graph-filter-reset-btn"
                      onClick={handleGraphResetFilters}
                      style={{ transition: 'color 0.2s ease' }}
                      //onMouseEnter={(e) => e.target.style.color = '#007bff'}
                      //onMouseLeave={(e) => e.target.style.color = 'inherit'}
                      data-tooltip-id="stage-2-graph-filter-reset-btn-tooltip"
                      data-tooltip-content="Clear Filters"
                      data-tooltip-place="top"
                    >
                      <FontAwesomeIcon icon={faTrash} size="md" />
                      <ReactTooltip id="stage-2-graph-filter-reset-btn-tooltip" style={{ zIndex: 9999 }} />
                    </button>
                  </>
                )}
                <button
                  className="btn btn-link text-dark p-0"
                  id="stage-2-graph-filter-close-btn"
                  onClick={toggleGraphFilterSidebar}
                  style={{ transition: 'color 0.2s ease' }}
                  //onMouseEnter={(e) => e.target.style.color = '#ff4d4d'}
                  //onMouseLeave={(e) => e.target.style.color = 'inherit'}
                  data-tooltip-id="stage-2-graph-filter-close-btn-tooltip"
                  data-tooltip-content="Close"
                  data-tooltip-place="top"

                >
                  <FontAwesomeIcon icon={faTimes} size="lg" />
                  <ReactTooltip id="stage-2-graph-filter-close-btn-tooltip" style={{ zIndex: 9999 }} />
                </button>
              </div>
            </div>
                ...
                return (
                  <div key={field} className="my-3">
                    ...
                    <Slider
                      ...
                      value={[
                        stage2Level2Filter.find(f => f.parameterName1 === field)?.value1 ?? min,
                        stage2Level2Filter.find(f => f.parameterName1 === field)?.value2 ?? max,
                      ]}
      ...
      {/* Modals */}
      <ModalComponent
        id="stage-2-confirm-large-data-fetch-modal"
        size="md"
      ...
      />
      <ModalComponent
        id="stage-2-confirm-tooltip-image-fetch-modal"
        size="md"
...
export default LayoutGraphDataStage2;
