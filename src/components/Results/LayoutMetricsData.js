 ...
 * All rights reserved.  
 *  
 * File Name: LayoutMetricsData.js  
 * Description: This file is responsible to show Layout Metrics Data 
 *  
...
const LayoutMetricsData = ({ gdsData = [], actionSelected, loadingRef, loadingMore }) => {
  ...
  return (
    <div>
      ...
      <div className="d-flex align-items-center justify-content-center">
        <div className="accordion w-100" id={`accordion-metrics-data`}>
          {gdsData?.items?.map((gds, index) => (
            <div className="accordion-item" key={index}>
              {/* react-tooltip  to show gds image on hover*/}
              <div
                data-tooltip-id={`tooltip-${gds?.name}`}
                data-tooltip-html={`<img src="${gds?.src}" style="width: 200px;" />`}
                data-tooltip-place="bottom"
              >
                <h2
                  className="accordion-header"
                  id={`layout-metrics-data-accordion-${gds?.name}`}
                >
                  <button
                    className="accordion-button collapsed"
                    id={`layout-metrics-btn-${gds?.name}`}
                    type="button"
                    data-bs-toggle="collapse"
                    data-bs-target={`#gds-metrics-data-body-${gds?.name}`}
                    aria-expanded="false"
                    aria-controls={`gds-metrics-data${gds?.name}`}
                  >
                        ...
                        {!isUserOnStage2Page && (
                          <div className="form-check">
                            <input
                              ...
                              type="checkbox"
                              value={gds.name}
                              id={`gds-metrics-data-checkbox-${gds?.name}`}
              ...
              />
              <div
                id={`gds-metrics-data-body-${gds?.name}`}
                className="accordion-collapse collapse"
                aria-labelledby={`heading${index}`}
                data-bs-parent="#accordion-metrics-data"
              >
                <div className="accordion-body">
                  <div style={{ width: "100%", height: "400px" }}>
                    {gds?.metricsData && gds?.metricsData?.length > 0 && (
                      <DataGrid
                        columns={[
                          ...Object.keys(gds?.metricsData[0])?.map((key) => {
                            const isNumber = typeof gds?.metricsData[0][key] === 'number';
                            return {
                              field: key,
                              headerName: key,
                              width: 150,
                        ...
                        rows={gds?.metricsData?.map((data, rowIndex) => ({
...
export default LayoutMetricsData;
