...
def _adjust_routing_tracks(tech):
    if tech.number_of_routing_tracks != None:
    ...
    if tech.backside_power_rail:
        if not minimum_cell_height <= tech.cell_height + tech.power_rail_width:
...
def main(args):
    ...
    def permuteHelper(count, toVals, layerMap_exporter):
        ...
        layerMap_exporter.exportLayerMap(tech, f"{args['output_dir']}/layerMap{args['cell']}_{count['clean']}_RT_{tech.number_of_routing_tracks}.json", list(io_pins))
    ...
    # shutil.move(".lib-utils.log", output_dir)
