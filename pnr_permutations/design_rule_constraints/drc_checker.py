...
class DesignRuleCheck(RuleChecker):
    def checkDRC(self, strict=True, update=False):
        ...
        if not self.checkconnections():
            return False
        if not self.checkVddVssPosition():
            return False
        ...
        if not self.checkLayerNanoSheetExtension(self.layerNumber['NmosNanoSheet'], self.layerNumber['NMOSGate'], self.layer_properties.gateSheetExtension):
            return False
        if not self.checkLayerNanoSheetExtension(self.layerNumber['NmosNanoSheet'], self.layerNumber['PMOSGate'], self.layer_properties.gateSheetExtension):
            return False
        if not self.checkLayerNanoSheetExtension(self.layerNumber['NmosNanoSheet'], self.layerNumber['NMOSInterconnect'], self.layer_properties.interconnectSheetExtension):
            return False
        if not self.checkLayerNanoSheetExtension(self.layerNumber['NmosNanoSheet'], self.layerNumber['PMOSInterconnect'], self.layer_properties.interconnectSheetExtension):
            return False
        ...
        if not self.checkLayerLayerGap(self.layerNumber['NmosNanoSheet'], self.layerNumber['VIA_PMOSInterconnect_NMOSInterconnect'], self.layer_properties.via600SheetGap, 'V'):
            return False
        if not self.checkLayerLayerExtension(self.layerNumber['M0'],self.layerNumber['PMOSInterconnect'],self.layer_properties.InterconnectExtensionfromM0):
            return False
        if not self.checkLayerLayerExtension(self.layerNumber['M0'],self.layerNumber['NMOSInterconnect'],self.layer_properties.InterconnectExtensionfromM0):
            return False
        if not self.checkLayerLayerExtension(self.layerNumber['M0'],self.layerNumber['NMOSGate'],self.layer_properties.GateExtensionfromM0):
            return False
        if not self.checkLayerLayerExtension(self.layerNumber['M0'],self.layerNumber['PMOSGate'],self.layer_properties.GateExtensionfromM0):
            return False        
        return True