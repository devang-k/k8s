...
class PermutationEngine:
    ...
    def __init__(self,gds_file,layer_map_dir):
        global layerNumber
        self.gds_reader = GDSReader(write_json=True)
        gds_data = self.gds_reader.read(gds_file)
        ...
        self.scaleH = round(self.units[0]*(10**9),2)
        self.scaleV = round(self.units[1]*(10**9),2)
        ...
        self.gds_jsons = {}
        drc_checker = {
            'gaa': GAADesignRuleCheck,
            'cfet': CFETDesignRuleCheck,
            'finfet': FINFETDesignRuleCheck
        }
        k = 10
        ...
        self.tech = None
        self.drc  = drc_checker[self.technology](self.layerMap,self.layer_properties, layerNumber)
    ...
    def addPatternCut(self, layer, datatype, elements, extraLayer = None):
        ...
        for poly in polygons:
            coords = list(simplify_by_angle(poly))
    ...
    def getOptimizedBounds(self, component):
        nanosheet = None
        component_top = self.globalBoundary[1]
        component_bottom = self.globalBoundary[3]
        ...
        if nanosheet:
            nanosheet_bounds = self.layerMap[layerNumber[nanosheet]].polygons[0].point.bounds
        ...
        if component.planarConnected:
            ...
            if component.layer in [layerNumber["PMOSGate"],layerNumber['PMOSInterconnect']]:
                #this hardcoding or the assumption of PMOS and NMOS should be removed in the later commits
                if self.flipped == 'Mx':
    ...
    def generatePermutations(self, polygons_to_permutate, limiter):
        ...
        def find_valid_permutations(num_tracks, num_metals, clashes_with, limit=1000000):
            ...
            def backtrack(metal):
                ...
                for track in range(num_tracks):
                    # Check if this metal can be placed on the track without conflicts
                    clash_list = clashes_with.get(metal, [])  
                    if all(
                        current_assignment[m] not in {track}.union(track_restrictions.get(m, set())) or 
                        m not in clash_list 
        ...
        def calculate_restricted_tracks(non_planar_layers, num_tracks):
            """Calculates restricted tracks based on orientation and PMOS/NMOS zone"""
            bottom_tracks = num_tracks//2
            upper_track_range = list(range(-(-num_tracks//2), num_tracks))
            lower_track_range = list(range(0, bottom_tracks))
            restriction = None
            for layer in non_planar_layers:
                ...
                if layer in [layerNumber["NMOSGate"],layerNumber['NMOSInterconnect']]:
                    restriction = 'NMOS'
            if self.flipped == 'Mx':
                if restriction == 'PMOS': return upper_track_range
                elif restriction == 'NMOS': return lower_track_range
            else:
                if restriction == 'PMOS': return lower_track_range
                elif restriction == 'NMOS':return upper_track_range
            return []
        ...
        for i in range(len(polygons_to_permutate)):
            ...
            if self.technology in ['gaa', 'finfet']: 
                ...
                if flag:
                    restricted_tracks = calculate_restricted_tracks(non_planar_layers, len(track_centers))
        ...
        self.lvs_client.run()
        # # LVS checking
        # self.lvs_handler(netlist_path, cell_name, tech, output_folder)
    ...
    if isfile(gds_file):
        ...
        if permutation_engine.drc.checkDRC(strict = False):
            ...
            if args.get("onlyOne", False):
                ...
                permutation_engine.generatePermutations(polygons_to_permutate)
